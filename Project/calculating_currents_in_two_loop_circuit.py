...
The program defines 4 separate functions:
...
def calculate_current_values(direction_values, voltage_values, resistance_values):
  ...
  try:
    ...
    current_amps = numpy.absolute(x)  #Final vector of current magnitudes
  except ValueError:
...
def plot_current_values(current_amps, final_direction_values):
  """Takes two lists of current magnitudes and directions, and creates graphic representing the directions of currents in the circuit with respect to Node 1 and labelling their values"""
  current_graph = plt.figure()
  
  if final_direction_values[0] == 'in':
    plt.arrow(-1,0,1,0,width=0.2)
  else:
    plt.arrow(0,0,-1,0,width=0.2)
  if final_direction_values[1] == 'in':
    plt.arrow(1,0,-1,0,width=0.2)
  else:
    plt.arrow(0,0,1,0,width=0.2)
  if final_direction_values[2] == 'in':
    plt.arrow(0,-1,0,1,width=0.2)
  else:
    plt.arrow(0,0,0,-1,width=0.2)
  plt.plot(0,0,marker='o',color='black')
  current_graph.text(0.5,0.25, 'i2 = ' + str(current_amps[1]) + 'Amps', horizontalalignment='center')
  current_graph.text(-0.5, 0.25, 'i1 = ' + str(current_amps[0]) + 'Amps', horizontalalignment='center')
  current_graph.text(0.25, -0.5, 'i3 = ' + str(current_amps[2]) + 'Amps')
...
print(plot_current_values(test_results_1[0], test_results_1[1]))